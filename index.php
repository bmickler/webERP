<?php
/**
 * Starting/Home page.  Directory index
 * 
 * @package webERP
 * @subpackage Core
...
/**
 * The module link codes are hard coded in a switch statement below to determine 
 * the options to show for each tab
 */
include('includes/MainMenuLinksArray.php');

/**
 * Show different content based on user type (supplier/customer)
...
/**
 * Load the desired application designated in the the URL
 */
...
while ($i < count($ModuleLink)){
    /**
     * This determines if the user has display access to the module see 
     * config.php and header.php for the authorisation and security code
     */
    if ($_SESSION['ModulesEnabled'][$i]==1)	{
        /**If this is the first time the application is loaded then it 
         * is possible that SESSION['Module'] is not set if so set it to 
         * the first module that is enabled for the user
         */
...
echo '</ul></div></div>'; // END <MainMenuDiv> ===HJ===
...
echo '</ul></div>';// END TransactionsDiv
...
function GetRptLinks($GroupID) {
    /** 
     * This function retrieves the reports given a certain group id as defined in 
     * /reports/admin/defaults.php in the acssociative array $ReportGroups[]. It 
     * will fetch the reports belonging solely to the group specified to create a 
     * list of links for insertion into a table to choose a report. Two table sections 
     * will be generated, one for standard reports and the other for custom reports.
     */
