...
/**
 * Adds a message to the global message array queue
 * 
 * @global ArrayObject|string[] $Messages Message array
...
/**
 * Reverses escaped string for HTML & database use
 * @param string $str
 * @return string
 */
...
/**
 * Checks if a string is a well-formed email address
 * @param string $Email
 * @return boolean
 */
function IsEmailAddress($Email){
    ...
    /**
     * Check for a DNS 'MX' or 'A' record.Windows supported from PHP 5.3.0 on, so check.
     * 
     * @todo Apparentely causes some problems on some versions - perhaps bleeding 
     * edge just yet, so commenting out for now until fixed.
...
/**
 * dumps a variable's contents to screen between <pre></pre> tags for debugging
 * @param string|null|ArrayObject|integer $var
 */
...
/**
 * Gathers European Central Bank currency exchange rates
 * 
 * Returns an array of several currency valuations as copared to the Euro if 
 * successful.  Returns FALSE if unsucessful.
 * 
 * @link http://www.ecb.int/stats/exchange/eurofxref/html/index.en.html daily 
 * details of the European Central Bank rates - published daily
 * @return boolean|float[]
 * @todo may want to move this and similar specialized functions to a class or file
 * of their own.  These type of functions which pull data from external websites may
 * need to be adjusted more frequently as those services change their offering/urls
 */
...
/**
 * Gathers the currency rate base on the user's profile setting for currency abbreviation
 * 
 * @param type $CurrCode
 * @param type $CurrenciesArray
 * @return int
 * @todo may want to move this and similar specialized functions to a class or file
 * of their own.  These type of functions which pull data from external websites may
 * need to be adjusted more frequently as those services change their offering/urls
...
function GetCurrencyRate($CurrCode,$CurrenciesArray) {
    if ((!isset($CurrenciesArray[$CurrCode]) 
            OR !isset($CurrenciesArray[$_SESSION['CompanyRecord']['currencydefault']])) 
...
/**
 * Conducts an Oanda currency exchange rate analysis. Oanda seems to have removed 
 * this functionality as of 25 Aug, 2018 in place of their paid API
 * 
 * @todo explore alternatives to make this work again
 * @link https://www.oanda.com/fx-for-business/exchange-rates-api Oanda's API information
...
/**
 * Conducts a Google currency exchange rate analysis. Google seems to have removed 
 * this functionality as of 25 Aug, 2018
 * 
 * @todo explore alternatives to make this work again
 * @param string $CurrCode
 * @return float
...
/**
 * Replaces LF (line feed, \r) and CR (carriage return, \n) characters with the 
 * ASCII charchter code 10 new line.  Helps correct new line discrepancies between
 * computing platforms.
 * @param string $str
 * @return string
 */
...
/**
 * Wiki hyperlink creation abstraction utility
 * 
 * Prints to screen a hyperlink formatted in the form used by the desired wiki 
 * software.
 * @param string $WikiType Type of wiki software to be used
 * @param string $WikiPageID Page name/id to be used when creating the link
 */
...
/**
 * Checks if a file exists by verifying that it can be opened
 * @param string $url
 * @return boolean
 */
...
/**
 * Converts numbers to the format of the user's locale
 * 
 * @global string $DecimalPoint The character used for a decimal point
 * @global string $ThousandsSeparator The character used for the Thousands separator
 * @param float $Number the floating point number to be converted
 * @param integer $DecimalPlaces number of significant decimal places to round the return value to
 * @return float
 */
function locale_number_format($Number, $DecimalPlaces=0) {
    ...
    if(substr($_SESSION['Language'], 3, 2)=='IN') {
        /**
         * If country is India (??_IN.utf8). See Indian Numbering System in 
         * Manual, Multilanguage, Technical Overview.
         */
...
/**
 * Attempts to parse the input of the user into useable number.  Returns a float
 * as a string if sucessful.  If unsucessful, returns a verbose error message.
 * 
 * @global string $DecimalPoint The character used for a decimal point
 * @global string $ThousandsSeparator The character used for the Thousands separator
 * @param type $Number
...
function filter_number_format($Number) {
    ...
    /**
     * It is possible, if the user entered the $DecimalPoint as a thousands separator 
     * and the $DecimalPoint is a comma, that the result of this could contain 
     * several periods (i.e., ".") so need to ditch all but the last "."
     */
    if (mb_substr_count($SQLFormatNumber,'.')>1){
        /**
         * there were too many periods ('.') detected, this number is messed up.
...
/**
 * Re-formats numbers to the Indian numbering system
 ...
 * Sends an email
 * 
 * @see includes/htmlMimeMail.php
 * @param htmlMimeMail $mail htmlMimeMail object
 * @param string $To Recipient(s), usually defined by GetMailList()
 * @return mixed
 */
function SendMailBySmtp(&$mail,$To) {
    ...
    if(IsEmailAddress($_SESSION['SMTPSettings']['username'])){
        /**
         * User has set their full email address as the user name
         */
        $SendFrom = $_SESSION['SMTPSettings']['username'];
    }else{
        /**
         * User only set it's name instead of full email address
         */
...
/**
 * Returns an array of email addresses for a specified email group.
 * 
 * Each element in the array is formatted in the following manner:
 *      Display Name <local-part@domain.tld>
 * 
 * @link https://www.ietf.org/rfc/rfc822.txt RFC 822
 * @param type $MailGroup Name of the mail group to retrieve recipient list for
 * @return string[]
 */
...
/**
 * 'Setter' function used in Z_ scripts to change one field across the table
 * 
 * @param string $TableName table to query
 * @param string $FieldName field being updated
 * @param mixed $OldValue Old value to be changed
 * @param mixed $NewValue New value to replace the old value
 */
...
/**
 * Returns an HTML <select> populated with <options> relevant to the selected reporting
 * period.  Used in report scripts for standard periods.
 * 
 * @param type $Choice the selected period to report on.  Periods are selected in 
 * the "Period" dropdown boxes and are derived from the 'periods' table in the database.
 * @param char $Options
 * @return string HTML select with option values for the selected period.
 */
...
/**
 * Determins the starting date (From) and ending date (To) for a user-specified
 * period.  Used in report scripts.
 * 
 * @param string $PeriodName descriptive name of the period to assess
 * @param string $FromOrTo the string 'From' (default) or 'To'
 * @return date Unix data-time value
 * @see includes/DateFunctions.inc::GetPeriod()
 */
